...
<?php
    ...
    $font  = "NimbusSans-Regular";    # Font Name
    $size  = 46;             # Font size
    ...
    $color_top = "#00FF00";      # Text Color (also names like 'red', 'blue', etc.)
    $size_top  = 26;             # Font size
    $antiAlias_top = true;      # Anti-Alias the text
    $msg_top = "Next Show";

    # Set php default timezone to /etc/timezone for local time
    ...
    # 31st - 6pm
    # Nov 1st - 7pm
    $Showtime = new DateTime('23:59');
    if (date('j') >= 31)
        $Showtime = new DateTime('17:30');

    elseif (date('j') >= 20)
        $Showtime = new DateTime('19:00');

    elseif (date('j') >= 8)
        $Showtime = new DateTime('19:30');

    elseif (date('j') <= 2) # November
        $Showtime = new DateTime('19:00');
	...
	$diff += 0; // Add offset to line up with first song countdown
